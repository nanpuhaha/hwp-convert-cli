...
- Prefer the repo CLI at `target\release\hwp-convert-cli.exe`; if it does not exist, build it with `cargo build --release`.
...
```powershell
target\release\hwp-convert-cli.exe convert .\input.hwp --to hwpx --overwrite --json
```

Single file to PDF:

```powershell
target\release\hwp-convert-cli.exe convert .\input.hwp --to pdf --overwrite --json
```

Directory batch conversion:

```powershell
target\release\hwp-convert-cli.exe convert .\docs --to pdf --out-dir .\converted --recursive --skip-existing --json
```
...
3. Build the CLI if `target\release\hwp-convert-cli.exe` is missing.
